 ...
 * 用于图片增加倒角
 */
class CornerTransform(context: Context?, radius: Float) :
    ...
    override fun transform(
        ...
        val rectF =
            RectF(0.0f, 0.0f, canvas.getWidth() as Float, canvas.height as Float)
        ...
        if (exceptRightTop) { //右上角不为圆角
            canvas.drawRect(canvas.getWidth() - radius, 0f, radius, radius, paint)
        }
