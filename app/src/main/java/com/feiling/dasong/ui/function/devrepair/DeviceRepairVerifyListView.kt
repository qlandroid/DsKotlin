 ...
 * @author ql
 */
class DeviceRepairVerifyListView(
    context: Context?,
    var page: DeviceRepairVerifyListContentPage,
    ...
    private fun initView() {
        ...
        mParamsModel.vstatus = page.state
    ...
    private fun loadData() {
        ...
        val subscribe = ServiceBuild.deviceRepairService.getPageList(
            ...
            .subscribe(
                { model ->
                    mAllList.add(model)
                    val type = when (model.vstatus) {
                        DeviceRepairVerifyListContentPage.UNVERIFY.state -> {
                            StatusTypeEnum.UNVERIFY
                        }
                        DeviceRepairVerifyListContentPage.VERIFY.state -> {
                            when (model.devState) {
                                ...
                                DeviceRepairListContentPage.END.state -> {
                                    var state =
                                        DevRepairModel.RepairState.getState(model.repairState)
                                    when (state) {
